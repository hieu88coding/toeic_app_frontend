...
function CreateGrammar({
    ...
    const uploadFile = (fileData) => {
        return new Promise((resolve, reject) => {
            ...
            const fileRef = ref(storage, `Grammars/${fileData.fileType}/${testTilte}/${fileData.file.name}`);
    ...
    const uploadAnswer = (fileData) => {
        return new Promise((resolve, reject) => {
            ...
            const fileRef = ref(storage, `Grammars/json/${testTilte}/${fileData.file.name}`);
            uploadBytes(fileRef, fileData.file)
                .then(async (snapshot) => {
                    const fileUrl = await getDownloadURL(snapshot.ref);
                    let fileDataWithUrl = {
    ...
    return (
        <Modal
            title={`Thêm mới bài Grammar`}
            open={isOpenModal}
            onCancel={closeModal}
            onOk={handleCreateGrammar}
