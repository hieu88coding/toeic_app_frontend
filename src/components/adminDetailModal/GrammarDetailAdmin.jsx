...
function GrammarDetailAdmin(props) {
    ...
    const createExel = async (preExel, partName) => {
        ...
        try {
            let exelRefs = await listAll(ref(storage, `Grammars/exel/${testName}/nouns${testName.charAt(4)}_question.xlsx`));
            ...
            await uploadBytes(ref(storage, `Grammars/exel/${testName}/nouns${testName.charAt(4)}_question.xlsx`), blob);
    ...
    const createAnswer = async () => {
        ...
        const excelBuffer = await workbook.xlsx.writeBuffer();
        try {
            let exelRefs = await listAll(ref(storage, `Grammars/exel/${testName}/nouns${testName.charAt(4)}_answer.xlsx`));
            await Promise.all(exelRefs.items.map(async (exelRef) => {
                await deleteObject(exelRef);
                console.log('Đã xóa file Excel cũ');
            }));
            const blob = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
            await uploadBytes(ref(storage, `Grammars/exel/${testName}/nouns${testName.charAt(4)}_answer.xlsx`), blob);
            console.log('Upload thành công');
        } catch (error) {
            console.log(error);
        }
...
export default GrammarDetailAdmin;